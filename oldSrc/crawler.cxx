...
{
  logger.trace("Recived completed request");
  logger.info("Queue size: " + std::to_string(request_queue.size()));
  
  RequestType type = RequestType::HEAD;
  ...
  if(r->get_timed_out())
  {
    ...
    //prepare_next_request(type);
    strand.post(bind(&Crawler::prepare_next_request, this, type));
  }
  ...
  if(r->get_server() != org_domain || r->get_path() != org_path)
  {
    if( (found = r->get_path().find("/robots.txt")) == 0)
    {
      ...
      strand.post(bind(&Crawler::process_robots, this, org_domain, 
        org_proto, r->get_data()));
      delete(r);
  ...
  if( (found = r->get_path().find("/robots.txt") ) == 0)
  {
    ...
    strand.post(bind(&Crawler::process_robots, this, r->get_server(), 
      r->get_protocol(), r->get_data()));
  ...
  if(r->get_request_type() == RequestType::HEAD)
  {
    ...
    //if(strand.post(bind(&Crawler::check_if_header_text_html, this, r->get_headers())))
    {
...
void Crawler::prepare_next_request(RequestType type)
{
  ...
  if(!request_queue.empty())
  {
    ...
    logger.info("Creating pointer with: " + domain + " " + path + " "
      + protocol);
...
{
  logger.trace("Sending request to client");
  strand.post(bind(&http_client::make_request, &client, request));
